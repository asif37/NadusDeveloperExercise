...
export class MachinesManagementComponent implements OnInit, AfterViewInit {
  ...
  machineList: Machine[];
  dailogRef: MatDialogRef<AddMachineComponent>;
  ...
  openAddFileDialog(machine: Machine = null) {
    this.dailogRef = this.dialog.open(AddMachineComponent, {
      id: 'addMachine',
      hasBackdrop: false,
      height: '500px',
      width: '800px',
    });
    if (this.dailogRef.componentInstance.machine) {
      this.dailogRef.componentInstance.machine = (machine) ? machine : new Machine();
    }
    if (this.dailogRef.componentInstance.emitter) {
      this.dailogRef.componentInstance.emitter
        .subscribe(data => {
          if (data === "close") {
            let dailogToClose = this.dialog.getDialogById('addMachine');
            dailogToClose.close();
            this.getMachineList();
          }
        });
